...
public class StateTest {
	...
	public static void main(String[] a) throws Exception {
		...
		for(int i=0;i<10;i++) s.setStatus(exp.get(i), i<7?ValidationState.OK:(i<9?ValidationState.MISSING:ValidationState.WRONG), i==9?"stone":null);
		s.endScan();
		check(s.isValidated() && s.getOk()==7 && s.getMissing()==2 && s.getWrong()==1, "scan: 7/2/1");
		check(Math.abs(s.getProgress()-0.7)<1e-9, "progress 0.7");
		check("stone".equals(s.getWrongBlockName(exp.get(9))), "wrong block name kept");
		...
		check(s.getOk()==7 && s.getMissing()==2, "transitions keep counters consistent");
		s.setStatus(exp.get(9), ValidationState.OK, null);
		check(s.getWrong()==0 && s.getWrongBlockName(exp.get(9))==null, "wrong -> ok clears name");
