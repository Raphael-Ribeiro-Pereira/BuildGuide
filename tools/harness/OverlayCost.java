...
public class OverlayCost {
	...
	public static void main(String[] a){
		...
		s.beginScan(exp); int i=0; for(long p: exp){ s.setStatus(p, (i%10==0)?ValidationState.WRONG:ValidationState.OK, "Stone"); i++; } s.endScan(); // 5000 wrong of 50k
		for(int rep=0;rep<3;rep++){ long t0=System.nanoTime(); List<Long> wrong=s.getPositions(ValidationState.WRONG); long t1=System.nanoTime(); NoopBuf b=new NoopBuf(); for(long p: wrong) cube(b, LocalPos.unpackX(p)+0.2, LocalPos.unpackY(p)+0.2, LocalPos.unpackZ(p)+0.2, 0.6); long t2=System.nanoTime();
			System.out.printf("5000 wrong of 50k: getPositions %.2f ms (scans the whole map), cube pushes %.2f ms (%d vertices)%n",(t1-t0)/1e6,(t2-t1)/1e6,b.n); }
	}
}
